...
/* TODOs:
 * - Organização do código a fornecer
 *   - Organizar código fornecido em pacotes.
 *   - Comentar classes e métodos.
 *   - Melhorar coesão dos métodos.
 *   - Gerar projeto-base a ser enviado para os alunos
 * - Desejáveis:
...
public class Tela implements InterfaceUsuario
{
    ...
    // HashMap para guardar a posição de cada item exibido
    ...
    private void criarPainelItens() {
        ...
        // cria o hashMap usado para guardar a posição dos componentes de itens no painel
        itens = new HashMap<>();
    }
    ...
    public void jogadorPegouItem(EntidadeGrafica item) {
        ...
        painelItens.add(rotuloNomeItem);
        
        // cria um rótulo com a imagem do item (redimensionando para o tamanho padrão de imagens de itens)
        JLabel rotuloImagemItem = new JLabel(new ImageIcon(redimensionarImagem(item.getImagem(), false)));
        // adiciona o rótulo da imagem no painel de itens
        painelItens.add(rotuloImagemItem);
        
        // guarda a posição do rótulo de nome do item (para que depois seja possível apagar o item da tela)
        itens.put(item.getNome(), painelItens.getComponentCount()-2);
    ...
    public void jogadorDescartouItem(EntidadeGrafica item) {
        // Obtém a posição do rótulo do item no painel (que havia sido guardada quando o item foi adicionado)
        int posicaoRotulo = itens.get(item.getNome());
        
        // Obtém os rótulos com o nome e a imagem do item
        Component rotuloNomeItem   = painelItens.getComponent(posicaoRotulo);
        Component rotuloImagemItem = painelItens.getComponent(posicaoRotulo+1);
        
        // Remove os rótulos com o nome e a imagem do item do painel de itens
        painelItens.remove(rotuloNomeItem);
        painelItens.remove(rotuloImagemItem);
        
        // Remove a posição do rótulo do item (já que agora ele não existe mais na tela)
